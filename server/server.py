...
#@app.route('/data/<path:path>') #need change
#def send_data(path):
 #   return send_from_directory('../data', path)

@app.route('/volume/<path:path>') #need change
def send_volume(path):
    return send_from_directory('../volume', path)

@app.route('/transfunc/<path:path>') #need change
def send_transferfunc(path):
    return send_from_directory('../transfunc', path)

@app.route('/surface/<path:path>') 
def send_surface(path):
    return send_from_directory('../surface', path)

@app.route('/data-description/<path:path>') 
def send_desc(path):
    return send_from_directory('../data-description', path)

@app.route('/quiz-image/<path:path>') 
def send_image(path):
    return send_from_directory('../quiz-image', path)
...
#            os.path.join(config.basedir, "quiz", "quiz.json"))
    parser = arg.ArgumentParser(description='Serve the VolumeVisual website.')
    # Flags go here
    ...
    if args.reset:
        ...
        compile("../quiz/questions/", "../quiz/volume_quiz.json")
